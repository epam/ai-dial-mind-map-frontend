...
export const Toolbar = () => {
  ...
  return (
    <div className="chat-header mx-7 flex min-h-[50px] items-center justify-end gap-4">
      <button
        className="toggle-mindmap-button group flex gap-1 transition-colors duration-200 hover:cursor-pointer hover:text-accent-primary"
      ...
      <button
        className="history-reset-button group flex gap-1 transition-colors duration-200 hover:cursor-pointer hover:text-accent-primary disabled:cursor-not-allowed disabled:text-secondary"
        onClick={() => dispatch(ChatUIActions.reset())}
        disabled={conversation.messages.length <= 2 || isPlayback}
      >
        <IconTrash
          size={18}
